//! The uptime_api_web crate contains the application's web interface which mainly are controllers implementing HTTP endpoints. It also includes the application tests that are black-box tests, interfacing with the application like any other HTTP client.
...
use uptime_api_config::{get_env, load_config, Config};
...
///
/// 1. Determine the environment the application is running in (see [`uptime_api_config::get_env`])
/// 2. Load the configuration (see [`uptime_api_config::load_config`])
